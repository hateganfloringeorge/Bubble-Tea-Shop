/* eslint-disable jsx-a11y/anchor-is-valid */
import React, { useContext } from 'react';
import { UserContext } from './UserContext';
import {HashRouter, NavLink, Link} from 'react-router-dom';
import Image from 'react-bootstrap/Image';
import slide05 from '../images/05.jpg';

const Footer = () => {

    const { user } = useContext(UserContext);

    return(
        <>
        <div className="footer-image">
            <Image src={slide05} fluid />
        </div>
        <footer id="footer" class="footer-1">
            <div class="main-footer widgets-dark typo-light">
                <div class="container">
                <div class="row">


                    <div class="col-xs-12 col-sm-6 col-md-3">
                        <div class="widget subscribe no-box">
                            <h5 class="widget-title">BUBBLE TEA<span></span></h5>
                            <p>An incredibly unique looking beverage, Bubble tea is a Taiwanese recipe made by blending a tea base with... <Link to="/about">see more</Link></p>
                        </div>
                    </div>



                    <div class="col-xs-12 col-sm-6 col-md-3">
                        <div class="widget no-box">
                            <h5 class="widget-title">Quick Links<span></span></h5>
                            <ul class="thumbnail-widget">
                            <HashRouter>
                                <li>
                                    <div class="thumb-content"><NavLink to="/"><a>Home</a></NavLink></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><NavLink to="/about"><a>About</a></NavLink></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><NavLink to="/menu"><a>Menu</a></NavLink></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><NavLink to="/contact"><a>Contact</a></NavLink></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><NavLink to="/review"><a>Review</a></NavLink></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><NavLink to="/faq"><a>FAQ</a></NavLink></div>
                                </li>
                                {!user ? (
                                    <li>
                                        <div class="thumb-content"><NavLink to="/login"><a>Login</a></NavLink></div>
                                    </li>
                                    ) : (
                                    <li>
                                        <div class="thumb-content"><NavLink to="/logout"><a>Logout</a></NavLink></div>
                                    </li> 
                                    )
                                }
                                {/*//TODO <li>
                                    <div class="thumb-content"><NavLink to="/review"><a>Review</a></NavLink></div>
                                </li> */}
                            </HashRouter>
                            </ul>
                        </div>
                    </div>



                    <div class="col-xs-12 col-sm-6 col-md-3">
                        <div class="widget no-box">
                            <h5 class="widget-title">Schedule<span></span></h5>
                            <p><span className="bold-text">Monday:</span> 10:00 - 20:00</p>
                            <p><span className="bold-text">Tuesday:</span> 10:00 - 20:00</p>
                            <p><span className="bold-text">Wednesday:</span> 10:00 - 20:00</p>
                            <p><span className="bold-text">Thursday:</span> 10:00 - 20:00</p>
                            <p><span className="bold-text">Friday:</span> 10:00 - 20:00</p>
                            <p><span className="bold-text">Saturday:</span> 12:00 - 20:00</p>
                            <p><span className="bold-text">Sunday:</span> 12:00 - 18:00</p>
                        </div>
                    </div>



                    <div class="col-xs-12 col-sm-6 col-md-3">
                        <div class="widget no-box">
                            <h5 class="widget-title">Contact Us<span></span></h5>
                            <ul class="social-footer2">
                            <li class=""><a title="youtube" rel="noopener noreferrer" target="_blank" href="https://www.youtube.com/watch?v=VPTeegrtASQ"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"></img></a></li>
                            <li class=""><a href="https://www.facebook.com/bubbleology/" rel="noopener noreferrer" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></img></a></li>
                            <li class=""><a href="https://twitter.com/BolaBolaBoba" rel="noopener noreferrer" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></img></a></li>
                            <li class=""><a title="instagram" rel="noopener noreferrer" target="_blank" href="https://www.instagram.com/explore/tags/bubbletea/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"></img></a></li>
                            </ul><br></br>
                            <p><span class="glyphicon glyphicon-earphone"></span> +40 746 901 983</p>
                            <p><span class="glyphicon glyphicon-envelope"></span> <a href="mailto:hateganfloringeorge@gmail.com" title="glorythemes">bubbletea@gmail.com</a></p>
                            <p><span class="glyphicon glyphicon-star"></span> Write a review here.</p>
                            <p><span class="glyphicon glyphicon-flag"></span>Bucharest, General Vasile Milea Boulevard, AFI Cotroceni Mall, 061344</p>
                        </div>
                    </div>


                </div>
                </div>
            </div>

            <div class="footer-copyright">
                <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <p>Copyright Bubble Tea Shop - Hategan Florin George &copy; 2020. All rights reserved.</p>
                    </div>
                </div>
                </div>
            </div>

        </footer>
        </>
    )
}

export default Footer;